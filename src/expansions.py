...
def lawler_expansion(pairs, y, method=1):
    # Not implemented yet
    ...
    # Number of nodes in new graph
    N_node = N_vertex + 2 * N_hyper_edges
    ...
    # Loop over hyperedges
    for hyperedge in hyper_edge_dict.keys():
        # Get vertices in hyperedge
        vertices = hyper_edge_dict[hyperedge]
        for vertex in vertices:
            # Add in-going edge
            edges.append((vertex, hyperedge))
            # Add out-going edge
            edges.append((hyperedge+N_hyper_edges, vertex)) # offset since out-going nodes appended at the end
        # Add connecting edge (from in-going node to out-going node)
        edges.append((hyperedge, hyperedge+N_hyper_edges))
    ...
    # Projection for added nodes
    # Loop over hyperedges
    for row in hyper_edge_dict.keys():
        # Find vertices belonging to the hyperedge
        vertices = hyper_edge_dict.get(row)
        divisor = len(vertices)
        if divisor > 0:
            for column in vertices:
                dense_Pv[row, column] = 1. / divisor # in-going node
                dense_Pv[row+N_hyper_edges, column] = 1. / divisor # out-going node
    ...
    # Approach 1: Remove nodes belonging to hyperedges
    if method == 1:
        # Directly project original vertices
        for i in range(N_vertex):
            dense_PvT[i, i] = 1.

    # Approach 2: Incorporate nodes belonging to hyperedges
    elif method == 2:
        # Loop over original vertices
        for row in range(N_vertex):
            # Find hyperedges that this vertex was a member of
            ...
            for hyper_edge in hyper_edge_dict.keys():
                if row in hyper_edge_dict.get(hyper_edge):
                    hyper_edge_membership.append(hyper_edge)
                    hyper_edge_membership.append(hyper_edge+N_hyper_edges)
            divisor = len(hyper_edge_membership) + 1  # +1 because self included
            for column in hyper_edge_membership:
                dense_PvT[row, column] = 1. / divisor
            dense_PvT[row, row] = 1. / divisor
